...
namespace Simplexcel.Tests
{
    public class SerializationTests
    {
        ...
        public void PageSetup_Serializes()
        {
            ...
            ws.PageSetup.Orientation = Orientation.Landscape;

            var cloned = Clone(ws);

            Assert.Equal(8, cloned.PageSetup.PrintRepeatColumns);
            Assert.Equal(12, cloned.PageSetup.PrintRepeatRows);
            Assert.Equal(Orientation.Landscape, cloned.PageSetup.Orientation);
        }
        ...
        public void SheetWithCell_Serializes()
        {
            ...
            Assert.Equal(CellType.Number, cloned.Cells["D4"].CellType);
        }
        ...
        public void SheetWithCellBorder_Serializes()
        {
            var ws = new Worksheet("Test");
            ws.Cells["A1"] = "My Test";
            ws.Cells["A1"].Border = CellBorder.All;

            var cloned = Clone(ws);

            Assert.True(cloned.Cells["A1"].Border.HasFlag(CellBorder.Bottom));
            Assert.True(cloned.Cells["A1"].Border.HasFlag(CellBorder.Left));
            Assert.True(cloned.Cells["A1"].Border.HasFlag(CellBorder.Right));
            Assert.True(cloned.Cells["A1"].Border.HasFlag(CellBorder.Top));
            Assert.True(cloned.Cells["A1"].Border.HasFlag(CellBorder.All));
        }
    }
}

...
namespace LamedalExcel.XlsxInternal
{
    internal static class StyleWriter
    {
        ...
        {
            var uniqueBorders = styles.Select(s => s.Border).Where(s => s != enCellBorder.None).Distinct().ToList();
            uniqueBorders.Insert(0, enCellBorder.None);
        ...
        private static void StyleAddCellXfsElement(XDocument doc, IList<XlsxCellStyle> styles, List<enCellBorder> uniqueBorders, List<string> numberFormats, IList<XlsxFont> fontInfos)
        {
        ...
        private static void StyleAddBordersElement(XDocument doc, List<enCellBorder> uniqueBorders)
        {
            ...
            foreach (var border in uniqueBorders)
            {
                ...
                if (border.HasFlag(enCellBorder.Left))
                {
                ...
                if (border.HasFlag(enCellBorder.Right))
                {
                ...
                if (border.HasFlag(enCellBorder.Top))
                {
                ...
                if (border.HasFlag(enCellBorder.Bottom))
                {
